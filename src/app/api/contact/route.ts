...
export async function POST(request: NextRequest) {
  try {
    ...
    // Extract fields matching the detailed evaluation form payload
    const {
      fullName,
      userType,
      organizationName,
    ...
    // Validation matching required structural form identifiers
    if (
      !fullName ||
      !workEmail ||
      !phoneNumber ||
    ...
    // Map listed services into a clean list presentation layout element
    const formattedServices =
      Array.isArray(servicesNeeded) && servicesNeeded.length > 0
        ? servicesNeeded.map((s: string) => `• ${escapeHtml(s)}`).join("<br>")
        : "None selected"

    // Formulate a structured section layouts mapping HTML body segments cleanly inside Tabular markup blocks
...
<table class=t122 role=presentation cellpadding=0 cellspacing=0 width=100% style="width:100%;"><tr><td class=t121><p class=t120 style="margin:0;Margin:0;font-family:Afacad,Arial,Helvetica,sans-serif;line-height:22px;font-weight:400;font-style:normal;font-size:12px;text-decoration:none;text-transform:none;direction:ltr;color:#888888;text-align:left;mso-line-height-rule:exactly;mso-text-raise:3px;">9, Osaro Isokpan Street, Lekki Phase 1, Lagos.</p></td></tr></table>
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: "Failed to dispatch notification to customer service" },
    ...
    return NextResponse.json(
      {
        success: true,
        message: "Project context processed and dispatched successfully",
      },
      { status: 200 },
    )
  } catch (error) {
    console.error("Evaluation form processing exception:", error)
    return NextResponse.json(
      { error: "Internal system processing failure" },
      { status: 500 },
    )
  }
}
